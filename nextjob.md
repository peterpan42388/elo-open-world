...
## Current Focus
- P2 Build Directory Hardening: run a live browser pass against the updated `Build` and `Market` action clusters on real project data, especially guest, pending-request, member, and endpoint-live states at tablet/mobile widths, and catch any remaining overflow or hierarchy issues that do not show up in fixture-based local checks

## Next Recommended Action
- run a live browser pass on `Build` and `Market` with real project data after the new primary/secondary action split, then trim any remaining long-string overflow or button hierarchy regressions that still weaken scanability on tablet or mobile widths
