...
test("onboarder install-plan and bootstrap report should align with setup-pack contract", async () => {
  ...
  assert.ok(plan.templates["run-bootstrap.sh"]);
  assert.ok(plan.templates["diagnose-bootstrap.sh"]);
  ...
  assert.equal(plan.steps.find((step) => step.id === "install-homebrew-runtime").command, "./install-homebrew.sh");
  ...
  assert.ok(report.templates["run-bootstrap.sh"]);
  assert.ok(report.templates["diagnose-bootstrap.sh"]);
  ...
  assert.ok(report.templates["report-status.sh"]);
